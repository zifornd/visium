...
pca <- function(seurat, assay = "SCT"){
  ...
  seuratpca <- RunPCA(seurat, assay = "SCT", verbose = FALSE)
...
umap <- function(seurat, reduction = "pca", dims = NULL, 
  ...
  seuratumap <- RunUMAP(seurat, reduction = reduction, dims = dims, 
...
tsne <- function(seurat, reduction = "pca", dims = NULL, 
  ...
  seurattsne <- RunTSNE(seurat, reduction = reduction, dims = dims, 
...
featurevln <- function(seurat, features = "nCount_Spatial", slot = NULL, log = T,
                       group.by = NULL, flip = FALSE, xlabs = TRUE, title = NULL){
  ...
  if (flip == TRUE){
    seuratvln <- seuratvln + coord_flip()
  }
  
  if(xlabs == FALSE){
    seuratvln <- seuratvln + theme(axis.text.x=element_blank())
  }
  return(seuratvln)
...
elbow <- function(seurat, ndims = 20, reduction = "pca", vline1 = 5, vline2 = 10, 
                  title = NULL){
  ...
  elbowplot <- ElbowPlot(seurat, ndims = ndims, reduction = reduction) +
    ...
    geom_vline(xintercept = vline2, linetype="dashed", 
               color = "red", size=0.5) + 
    geom_rect(aes(xmin=vline1, xmax=vline2, ymin=-Inf, ymax=Inf), alpha= 0.002) +
    labs(title=title)
