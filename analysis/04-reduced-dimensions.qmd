...
It can also be a very useful exploratory data analysis step to look at the PCA coloured by QC values such as overall counts (`nCount_Spatial`) and number of features per spot (`nFeature_Spatial`). The idea here is that a cluster may be driven by a technical reason such as a difference in library size which could be amenable to normalisation approaches. Other things like a known batch could be plotted as a labelled PCA plot for diagnosis of potential batch effects.
...
pca_plots_feature <- lapply(seurat, featureplot, reduction = "pca", 
...
Below we take number of PCs from visual inspectiondefined by the parameter elbow vector:
...
Below we run UMAP with the paramaters of choice.
...
Below we plot the UMAP of choice
...
umap_plots_feature <- lapply(seurat, featureplot, reduction = "umap", 
                             label = FALSE, feature = "nCount_Spatial")
```
...
::: callout-note
Please take the values for `perplexity` that are representative across TSNE plots and set them as params.
:::

Below we run TSNE with the paramaters of choice.
...
Below we plot each individual TSNE.
...
tsne_plots_feature <- lapply(seurat, featureplot, reduction = "tsne", 
                             label = FALSE, feature = "nCount_Spatial")
```
