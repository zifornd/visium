...
vlns <- lapply(meta, function(x, combinedQC, clean_label) {
  ...
  }, combinedQC = combinedQC, clean_label = trim_label)
```
...
bars <- lapply(c("Number.of.Reads", metricscols), function(x, metrics, clean_label) {
  
  barplotgg(metrics, ylab = x, xlab = "Sample.ID", colour_by = "Sample.ID", 
            clean = 11, delim = "_", xdelim = ".", ydelim = ".", 
            clean_label = clean_label)

  }, metrics = metrics, clean_label = trim_label)
```
...
list(
Parameter = "seurat",
Value = "[seurat](../../output/02-quality-control.rds)",
