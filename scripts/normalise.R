...
plotVariableGenes <- function(seurat, top = 10, selection.method = "sct"){
  ...
  top10 <- head(VariableFeatures(seurat), top)
  
  # plot variable features with and without labels
  plot1 <- VariableFeaturePlot(seurat, selection.method = selection.method)
...
mergeSamples <- function(seurat){
  ...
  for (i in 1:(length(seurat)-1)){
    ...
    print(paste0("Merging: ", names(seurat[[i]])[3], " ", names(seurat[[i+1]])[3]))
...
regressOut <- function(seurat, vars.to.regress = "batch", do.scale = FALSE, do.center = FALSE){
  
  seuratscaled <- ScaleData(seurat, vars.to.regress = vars.to.regress,
...
normaliseSctransform <- function(seurat, slot = "counts"){
  ...
  set.seed(0305)
  s <- GetAssayData(object = seurat, slot = slot)
  vst_out <- sctransform::vst(s, latent_var = c("log_umi"), return_gene_attr = TRUE, 
...
plotModelSctransform <- function(seurat, features, vst_outlist, plot_residual = TRUE,
                                 slot = "counts"){
  ...
  s <- GetAssayData(object = seurat, slot = slot)
  p <- sctransform::plot_model(vst_out, s, features, plot_residual = plot_residual)
