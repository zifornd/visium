...
plotpHeatmap <- function(genes_int, seuratprep, seurat_sub, order = "Cluster", 
  ...
  mycolan <- data.frame(Cluster = seuratprep@meta.data$Cluster, Sample = seuratprep@meta.data$Sample)
  ...
  p <- pheatmap(r[,cell_order], color = col, cluster_rows = F, cluster_cols = F, 
