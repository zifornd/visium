...
params:
    ...
    custom_sample_names: ["V1_Breast_Ca_1", "V1_Breast_Ca_2"]
---
...
if (length(params$custom_sample_names) > 0) {

  custom_names <- c(params$custom_sample_names, custom_names)

} else {

  custom_names <- c(metrics$Sample.ID, custom_names)

}

names(custom_names) <- c(metrics$Sample.ID, meta)
...
custom_names <- c("Number.of.Reads", metricscols)
...
names(custom_names) <- c(metrics$Sample.ID, metricscols)
...
names(custom_names) <- c(metricscols)
...
v1 <- violingg(map_metr, ylab = "Percentage", xlab = "QC", 
               ...
               custom_lab = custom_lab, clean_label = clean_label)

v2 <- violingg(map_metr, ylab = "Percentage", xlab = "QC", 
               colour_by = "Sample.ID", show_median = T, ylim = c(0,1), 
               clean = 20, custom_lab = custom_lab, clean_label = clean_label)
```
