...
As discussed above generally speaking we link to keep k.param as Seurat default (`k.param = 20`), therefore below we will keep it stable whilst changing the `resolution` parameter. We will save the results of this clustering into the seurat object so we can refer to this clustering downstream.
...
Another way of deciding what resolution to choose is to look at the stability of clusters across different resolution paramaters - in particular watching how spots move between clusters as the number of clusters increases. Below we use the package `clustree` to produce clustering trees to visualise this process.
...
@fig-spatial-res1 Plot spatial plot clusters for 'params$cluster.plot[[1]]'
...
@fig-spatial-res2 Plot spatial plot clusters for 'params$cluster.plot[[2]]'
...
Below we plot the UMAP with clustering resolution of choice.
