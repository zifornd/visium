...
```{r}
seurat <- readRDS("/sbgenomics/output-files/06-cell-annotation.rds")
```

## Integrate samples

```{r, warning=FALSE, message=FALSE}
seurat <- seuratIntegrate(seurat)
```
...
#### Res 0.2 and 0.3
```{r, warning=FALSE, message=FALSE}
sdplot1 <- SpatialDimPlot(seurat, label = TRUE, label.size = 3, group.by = "res.0.2")
sdplot2 <- SpatialDimPlot(seurat, label = TRUE, label.size = 3, group.by = "res.0.3")
```
...
```{r}
seurat <- addLabel(seurat, chosenCluster = "res.0.2")
```
...
```{r}
saveRDS(seurat, file = "/sbgenomics/output-files/08-integrate-samples.rds")
```
