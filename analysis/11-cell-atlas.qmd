...
if (params$vars.to.regress != FALSE) {
  ...
  atlas <- SCTransform(atlas, ncells = params$ncells.sct, variable.features.n = params$nfeatures.sct, 
                       verbose = FALSE, vars.to.regress = params$vars.to.regress)  
  
  atlas <- pca(atlas)
  
  featureplot(atlas, reduction = "pca", label = FALSE, feature = params$vars.to.regress, 
...
predictions.assay <- TransferData(anchorset = anchors, refdata = atlas@meta.data[[params$atlas.group]], prediction.assay = TRUE,
                                  weight.reduction = seurat[["pca"]], dims = dims)
```
