...
createDT <- function(sample_name, res) {
  ...
  dt <- DT::datatable(res[[sample_name]], class = 'cell-border stripe', extensions = 'Buttons', 
...
createDTFea <- function(sample_name, res, unlist = FALSE, html = FALSE, targets = c(11, 12)) {
  ...
  if (unlist) {
    ...
    resbind <- res_gsea_custom[sapply(res, nrow) > 0]

    # bind into single results table
    resbind <- bind_rows(resbind, .id = params$group)
    
  } else {

    # remove empty elements for clusters without any significant results
    resbind <- res_gsea_custom[sapply(res[[sample_name]], nrow) > 0]
