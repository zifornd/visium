...
# Annotate seurat object metadata (each spot) with T/F if meets threshold of gene expression
# Labels column based on some user defined name
# Can take multiple genes at once - T when both genes meet threshold

annotate <- function(seurat, features, anno, assay = "Spatial", slot = "data",threshold = 1, other = "Other"){
  
  # ready data

  seurat[[features]] <- as.data.frame(as.matrix(FetchData(seurat, vars = features, slot = slot)) > threshold)

  seurat[[anno]] <- apply(seurat[[features]], 1, function(x) ifelse(T %in% x & !F %in% x, anno, other))

  # remove feature cols from metadata - as this can affect downstream retrieval of features
...
annotate_highlow <- function(seurat, features_high, features_low, anno, slot = "data", threshold = 0.5, other = "Other"){
  
  seurat[[features_high]] <- as.data.frame(as.matrix(FetchData(seurat, vars = features_high, slot = slot)) > threshold)

  seurat[[features_low]] <- as.data.frame(as.matrix(FetchData(seurat, vars = features_low, slot = slot)) < threshold)
  
  seurat[[anno]] <- apply(seurat[[c(features_high, features_low)]], 1, function(x) ifelse(T %in% x & !F %in% x, anno, other))
...
defineExpressed <- function(seurat, features, assay = "Spatial", slot = "data", threshold = NULL){
  
  # ready data
  
  if(features %in% rownames(GetAssayData(object = seurat, slot = slot))){
    
    df <- as.data.frame(t(as.data.frame(GetAssayData(object = seurat, slot = slot)[features, ,drop=F])))
  ...
  if(!is.null(threshold)){
    ...
    p <- ggplot(df2, aes(x=Counts, color=Feature)) + 
      geom_density() + 
      geom_vline(aes(xintercept=threshold, color=Feature), linetype="dashed") + 
